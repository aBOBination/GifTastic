...
var gifTastic = {
    addButton : function (gif) {
        var btn = $("<button>").addClass("btn btn-display m-1").attr("topic", gif).text(gif)
    ...
    displayGifs : function(gif) {
        ...
        $.ajax({
            ...
            for(var i = 0; i < data.length; i++) {
                ...
                var imgdiv = $("<img>").addClass("card-img-top").attr({"src" : imgSrcStill, "data-still" :  imgSrcStill, "data-animate" :  imgSrcAnimated, "data-state" : "still"})
                var imgBody = $("<div>").addClass("card-body")
                var titleDiv = $("<p>").addClass("card-title").html("<strong>" + imgTitle + "</strong>")
